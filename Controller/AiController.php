...
class AiController {
    ...
    public function chat(): void {
        ...
        echo json_encode([
            ...
            'source'      => ($this->apiKey !== '') ? 'gemini' : 'local',
        ]);
        exit;
    }
    ...
    private function analyserGemini(string $nom, array $d, array $ingredients, array $profils, array $profil = []): ?string {
        ...
        $ch = curl_init(self::GEMINI_URL . '?key=' . $this->apiKey);
    ...
    public function recommander(): void {
        ...
        echo json_encode([
            ...
            'source'         => ($this->apiKey !== 'VOTRE_CLE_GEMINI_ICI') ? 'gemini' : 'local',
        ]);
        exit;
    }
    ...
    public function edamam(): void {
        ...
        // ── Clés Edamam ───────────────────────────────────────────────────────
        $appId  = defined('EDAMAM_APP_ID') ? trim((string) EDAMAM_APP_ID) : '';
        $appKey = defined('EDAMAM_APP_KEY') ? trim((string) EDAMAM_APP_KEY) : '';

        if ($appId === '' || $appKey === '') {
            echo json_encode(['error' => 'Clés Edamam manquantes. Vérifiez EDAMAM_APP_ID et EDAMAM_APP_KEY dans .env.']); exit;
        }
        ...
        if ($httpCode !== 200) {
            $apiError = json_decode((string) $result, true);
            $apiHint = '';
            if (is_array($apiError) && !empty($apiError['message'])) {
                $apiHint = ' - ' . (string) $apiError['message'];
            }
            echo json_encode(['error' => 'Edamam indisponible (code ' . $httpCode . ')' . $apiHint . '.']); exit;
        }
