...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($programId === '' || $name === '' || $reps === '' || $sets === '') {
        ...
        $exercise = new Exercise((int) $programId, $name, (int) $reps, (int) $sets);
        $exerciseC = new ExerciseC();
        $exerciseC->create($exercise);
    }
}
?>
                ...
                <form id="exerciseForm" method="post" action="">
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="program_id" id="program_id" required>
                            ...
                            <?php foreach ($programs as $program) { ?>
                                <option value="<?= htmlspecialchars($program['id']) ?>"><?= htmlspecialchars($program['name']) ?> - level <?= htmlspecialchars($program['level']) ?></option>
                            <?php } ?>
                        </select>
                    ...
                    <div class="mb-3 row">
                        <div class="col-md-6">
                            ...
                            <input type="number" class="form-control" name="reps" id="reps" min="1" step="1" required>
                            <div class="text-danger small" id="repsError"></div>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Sets</label>
                            <input type="number" class="form-control" name="sets" id="sets" min="1" step="1" required>
